...
En este instante, el sistema operativo divide la ejecución. Se crean dos procesos de ejecución distintos. Usamos el valor de pid para decirle al CPU qué código debe ejecutar el proceso padre y cuál el proceso hijo.
...
El proceso padre se ejecuta en este bloque y lo que hace es incrementar su propia copia de la variable en pasos de 10 hasta 100. El ``wait`` asegura que el padre recoja el "estado de salida" del hijo, permitiendo que el sistema operativo libere los recursos del hijo correctamente.
...
Al abrir la hoja de texto podemos ver el hijo ha escrito que la variable es 0 en las 10 iteraciones ya que como se mencionó anteriormente la memoria no es compartida, por lo que el proceso hijo presenta su propia copia de "variable".
...
Gracias a este ejercicio se demuestra que los procesos en Linux son independientes. A pesar de que el código parece indicar que ambos usan la misma variable, la salida donde el padre llega a 100 y el hijo registra 0, confirma que el Sistema Operativo protege la memoria de cada proceso individualmente.
